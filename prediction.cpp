...
bool Hooks::InPrediction( ) {
	...
	if( g_cl.m_processing ) {
		bool kick     = g_menu.main.visuals.removals.get( 0 );
		bool punch = g_menu.main.visuals.removals.get( 1 );

		if( stack.ReturnAddress( ) == CalcPlayerView_ret1 && ( kick || punch ) )
			return true;

		if( stack.ReturnAddress( ) == CalcPlayerView_ret2 && g_menu.main.visuals.removals.get( 1 ) ) {
			...
			if( angles ) {
				...
				if ( kick ) {
					*angles -= ( g_cl.m_local->m_aimPunchAngle() * g_csgo.weapon_recoil_scale->GetFloat() ) * g_csgo.view_recoil_tracking->GetFloat();
				}
			}
